...
export const EventList: React.FC<EventListProps> = ({ events }) => {
  ...
  const getEventCategoryInfo = (
    ...
    if (lowerDesc.includes("#food"))
      return { className: "category-food", label: "Food" };
    if (lowerDesc.includes("#discussion"))
      return { className: "category-discussion", label: "Discussion" };
    if (lowerDesc.includes("#teambuilding"))
      return { className: "category-teambuilding", label: "Team Building" };
    if (lowerDesc.includes("#standup"))
      return { className: "category-standup", label: "Standup" };
    if (lowerDesc.includes("#important"))
      return { className: "category-important", label: "Important" };
  ...
  return (
    <div className="event-list">
      {events.map((event, index) => {
        ...
        const previousDayString = previousEvent
          ? format(previousEvent.startDate, "EEEE, MMMM do")
          : "";
        ...
        const assignees = assigneeMatch
          ? assigneeMatch.map((s) => s.substring(1))
          : [];

        const cleanTitle = event.title.replace(/@\w+/g, "").trim();
        const isNextUp = index === 0;
        const { className: categoryClass, label: categoryLabel } =
          getEventCategoryInfo(event.description);
        ...
        return (
          <React.Fragment key={index}>
            ...
            <div
              className={`event-card ${isNow ? "active-event" : ""} ${
                isNextUp ? "next-up-event" : ""
              } ${categoryClass}`}
            >
